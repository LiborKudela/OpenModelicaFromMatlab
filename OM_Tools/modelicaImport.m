...
%     end
    ...
    % (not originall!! - Added by Libor Kudela!! - cpu time has dollar sign
    % and causes errors -> regexprep it out)
    name=regexprep(name,'\$','');
...
%             if ~all(dat(1,:)==dat(1,1)); error('foo'); end
            ...
            [tempRes, des, newRow] = assembleObject(row, level, tree, dataInfo, data, varargin{:});
%             if row==2289
%                 disp('');
%             end
            % Allocate memory for the object array
            foo = fieldnames(tempRes);
            foo = [foo'; cell(1, numel(foo))];
            ...
            res(getIndex(row, level, tree)) = tempRes;
            for i = 2:prod(dim)
                idx = getIndex(newRow, level, tree);
                [res(multDimIdx([dim 1],idx)), des, newRow] = assembleObject(newRow, level, tree, dataInfo, data, varargin{:});
            end
